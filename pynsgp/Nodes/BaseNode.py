...
class Node:	# Base class with general functionalities
	...
	def GetLatexExpression( self ):
		human_expression = self.GetHumanExpression()
		# add linear scaling coefficients
		'''
		if (self.ls_a != 0 or self.ls_b != 1):
			a = np.round(self.ls_a,2)
			b = 0
			jj = 2
			while b == 0 and jj < 16:
			  b = np.round(self.ls_b, jj)
			  jj += 1
			if self.ls_b < 0:
			  human_expression = "{}{}*({})".format(
			    a, b, human_expression
			  )
			else:
			  human_expression = "{}+{}*({})".format(
			    a, b,
